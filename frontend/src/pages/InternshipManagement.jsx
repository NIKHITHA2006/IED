...
const InternshipManagement = () => {
  ...
  const [form, setForm] = useState({ title: '', industry: '', requiredSkills: [], duration: '', description: '' });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ title: '', industry: '', requiredSkills: [], duration: '', description: '' });
    setModalOpen(true);
  };
    ...
    <div className="space-y-6 fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{internships.length} active placement listings</p>
        </div>
            ...
            {filtered.map((intern, idx) => (
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    <h3 className="text-base font-bold text-white leading-tight">{intern.title}</h3>
                  ...
                  <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                    <button onClick={() => openEdit(intern)} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-200 hover:bg-white/5 transition-colors">
                ...
                <p className="text-slate-400 text-sm leading-relaxed line-clamp-2 mb-4">{intern.description}</p>
      ...
      <AnimatePresence>
        {isModalOpen && (
          ...
          >
            ...
            >
              <h3 className="text-lg font-bold text-white mb-6">{editing ? 'Edit Internship' : 'New Internship'}</h3>
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 max-h-40 overflow-y-auto">
                    {skills.map((s) => (
                      <label key={s._id} className={`flex items-center gap-2 px-3 py-2 rounded-lg border cursor-pointer transition-colors text-xs ${form.requiredSkills.includes(s._id) ? 'border-indigo-500/50 bg-indigo-500/10 text-slate-200' : 'border-white/8 text-slate-400 hover:bg-white/5'}`}>
